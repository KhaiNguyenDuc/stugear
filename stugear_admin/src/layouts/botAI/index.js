...
function BotAI() {
  ...
  useEffect(() => {
    ...
    const getStatus = async () => {
      try {
        ...
        setAutoReview(response?.data?.is_auto_reviewed);
        setAutoReply(response?.data?.is_auto_reply_thread)
      } catch (error) {
        console.error("Error fetching validations:", error);
      }
    };
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 500,
      renderCell: (params) => (
        <Product id={params.row.product.id} name={params.row.product.title} />
      ),
    },
    {
      field: "status",
          ...
          {params.row.status.status == 3 && (
            <SoftButton
              ...
              onClick={() =>
                updateStatus(
                  params.row.id,
                  params.row.product.id ? params.row.product.id : params.row.thread.id,
                  1,
                  params.row.product.id ? "product" : "thread"
                )
              }
            >
              {"Duyệt"}
          ...
          <Card>
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
              <SoftTypography variant="h6">
                <>
                  <span style={{ marginRight: "8px" }}>Tự động duyệt bài đăng, sản phẩm:</span>
                  <SoftBadge
                    variant="gradient"
                    badgeContent={autoReview == 1 ? "Hoạt động" : "Tạm ngừng"}
                    color={autoReview == 1 ? "success" : "warnning"}
                    size="xs"
                    container
                  />
                     <SoftButton
              ml={2}
              variant="text"
              color={autoReview == 1 ? "error" : "success"}
              onClick={() =>
                updateConfig({reviewStatus: !autoReview, type: "review"})
              }
            >
              {autoReview == 1 ? "Tạm ngừng" : "Hoạt động"}
            </SoftButton>
                </>
              </SoftTypography>
            </SoftBox>
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" py={3} pl={3}>
              <SoftTypography variant="h6">
                <>
                  <span style={{ marginRight: "8px" }}>Tự động phản hồi bình luận bài đăng:</span>
                  <SoftBadge
                    variant="gradient"
                    badgeContent={autoReply == 1 ? "Hoạt động" : "Tạm ngừng"}
                    color={autoReply == 1 ? "success" : "warnning"}
                    size="xs"
                    container
                  />
                     <SoftButton
              ml={2}
              variant="text"
              color={autoReply == 1 ? "error" : "success"}
              onClick={() =>
                updateConfig({replyStatus: !autoReply, type: "reply"})
              }
            >
              {autoReply == 1 ? "Tạm ngừng" : "Hoạt động"}
            </SoftButton>
                </>
              </SoftTypography>
            </SoftBox>
